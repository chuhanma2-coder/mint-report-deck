...
if (budget.constraint && !["exact", "maximum", "flexible", "minimum-needed"].includes(budget.constraint)) errors.push("pageBudget.constraint 无效");
if (["exact", "maximum"].includes(budget.constraint) && budget.overflowPolicy !== "block") errors.push(`${budget.constraint} 页数合同必须设置 overflowPolicy=block`);
if (budget.constraint === "exact" && budget.planned !== budget.requested) errors.push("exact 页数合同中 planned 必须等于 requested");
if (budget.constraint === "maximum" && budget.planned > budget.requested) errors.push("maximum 页数合同中 planned 不得超过 requested");
